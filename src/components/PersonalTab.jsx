...
const DECOMP_KEYS = ["market_income", "taxes", "benefits", "pension_contributions", "purchasing_power"];
const DECOMP_META = {
...
export default function PersonalTab() {
  ...
  const [draftPensionContrib, setDraftPensionContrib] = useState(0);
  ...
  const handleCalculate = useCallback(async () => {
    ...
    const requestBody = {
      ...
      pension_contributions: draftPensionContrib,
    ...
    draftStudentLoan, draftHasPostgrad, draftLoanBalance, draftPensionContrib,
  ]);
              ...
              <div className="controls-row controls-row-6">
                ...
                <div className="control-item control-span-2">
                  ...
                  <div className="salary-input-wrapper">
                    ...
                    <input
                      type="number"
                      value={draftPensionContrib}
                      onChange={(e) => setDraftPensionContrib(parseFloat(e.target.value) || 0)}
                      min={0} step={50}
                    />
                  </div>
                </div>
                ...
                <div className="decomp-breakdown">
                  ...
                  <table className="decomp-table">
                    ...
                    {(() => {
                      ...
                      {["market_income", "taxes", "benefits", "pension_contributions"].map((key) => {
                        ...
                        return (
                          <tr key={key}>
                            ...
                            <td className="decomp-table-value">{rv ? formatSigned(rv.baseline) : "—"}</td>
                            <td className="decomp-table-value">{rv ? formatSigned(rv.reform) : "—"}</td>
                            <td className={`decomp-table-change ${change > 0 ? "positive" : change < 0 ? "negative" : "zero"}`}>
                          ...
                          <tr className="decomp-table-net-row">
                            ...
                            <td className="decomp-table-value font-semibold">
                              {rounded?.net_income ? formatSigned(rounded.net_income.baseline) : "—"}
                            </td>
                            <td className="decomp-table-value font-semibold">
                              {rounded?.net_income ? formatSigned(rounded.net_income.reform) : "—"}
                            </td>
                            <td className={`decomp-table-change font-semibold ${nominalChange > 0 ? "positive" : nominalChange < 0 ? "negative" : "zero"}`}>
                      ...
                      <tr className="decomp-table-purchasing-row">
                        ...
                        <td className="decomp-table-value"></td>
                        <td className={`decomp-table-change ${displayedPP > 0 ? "positive" : displayedPP < 0 ? "negative" : "zero"}`}>
                          {formatCurrency(displayedPP)}
                        </td>
                      </tr>
                  ...
                  <details className="methodology-details mt-3">
                    ...
                    <ul className="text-xs text-gray-400 mt-2 leading-relaxed list-disc pl-4 space-y-1">
                      ...
                      <li>Market income includes employment, self-employment, pension, savings, dividend, property, and other private income</li>
                      <li>Pension contributions includes both your personal contributions and any employer-matched employee contributions</li>
                      <li>Market income, taxes, benefits, pension contributions, and net income (nominal) are shown in <strong>cash terms</strong> for that year</li>
