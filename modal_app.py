"""Modal deployment for Spring Statement 2026 Personal Calculator API.
...
image = (
    ...
    .add_local_dir(
        str(Path(__file__).parent / "src" / "spring_statement_data"),
        remote_path="/root/spring_statement_data",
    )
)
...
def fastapi_app():
    ...
    Simulation(
        situation={
            "people": {
                "a": {"age": {2026: 30}, "employment_income": {2026: 30000}}
            },
    ...
    from spring_statement_data.spring_statement import (
    ...
    api = FastAPI(
        title="Spring Statement 2026 Personal Calculator API",
        version="1.0.0",
    )
    ...
    async def root():
        return {
            "status": "ok",
            "service": "spring-statement-calculator",
            "version": "1.0.0",
        }
    ...
    async def spring_statement(data: SpringStatementInput):
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Calculation error: {e}"
            )
